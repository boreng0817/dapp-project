...
const _shareRoom = async (name, location, price) => {
  ...
  alert("등록");
  // page update is done by shareRoom function
}
...
const _getMyRents = async () => {
  ...
  myRents = await contract.methods.getMyRents().call({from:user, gas: 3000000});
...
const _getAllRooms = async () => {
  ...
  const contract = await getRoomShareContract();
  const room_count = await contract.methods.roomId().call();
  
  rooms = Array(parseInt(room_count));

  for (let i = 0; i < room_count; ++i) {
    let room = await contract.methods.roomId2room(i).call();
    rooms[i] = room;
  }
...
const _recommendDate = async (roomId, checkInDate, checkOutDate) => {
  ...
  var date = await contract.methods.recommendDate(roomId, checkInDate, checkOutDate).call(
    {from:user, gas:3000000});

  const start = dateFromDay(currentYear, parseInt(date[0])).toDateString();
  const end = dateFromDay(currentYear, parseInt(date[1])).toDateString();

  message = `This room is rented in ${start} ~ ${end}`;
...
const getRoomRentHistory = async () => {
  ...
  try {
    room_idx = returnOptionsJSON().id;
  } catch (e) {
    return history;
  }

  history = await contract.methods.getRoomRentHistory(room_idx).call({from:user, gas: 3000000});
...
const markRoomAsInactive = async () => {
  ...
  const message = `Room ${roomId} is inactive now`;
  alert(message);
  _updateRooms();
}
...
const intializeRoomShare = async () => {
  ...
  const message = `Room ${roomId} is initialized`;
  alert(message);
  _updateRents();
}
...
const _markRoomAsInactive = async (_roomId) => {
  ...
  const contract = await getRoomShareContract();
  await contract.methods.markRoomAsInactive(_roomId).send({from:user});
}
